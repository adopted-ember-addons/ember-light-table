...
module('Integration | Component | light table | occlusion', function (hooks) {
  ...
  test('scrolled to bottom', async function (assert) {
    ...
    assert.ok(
      findAll('.vertical-collection tbody.lt-body tr.lt-row').length < 30,
      'only some rows are rendered'
    );

    let scrollContainer = find(
      '.lt-scrollable.tse-scrollable.vertical-collection'
    );
    assert.ok(scrollContainer, 'scroll container was rendered');
    let { scrollHeight } = scrollContainer;
    assert.ok(
      scrollHeight > 1500,
      'scroll height is 50 rows * 30 px per row + header size'
    );
